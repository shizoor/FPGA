...
class vgasignal(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        with m.If(self.v_line <= imgarr.shape[0]+frontvpl):
            with m.If(self.v_line >= frontvpl):
                with m.If (self.h_timer > fronthpl):
                    with m.If (self.h_timer < imgarr.shape[1]+fronthpl):   
                        m.d.sync+=p.eq(imgarr.shape[1]*(self.v_line-frontvpl)+(self.h_timer-fronthpl+2))
