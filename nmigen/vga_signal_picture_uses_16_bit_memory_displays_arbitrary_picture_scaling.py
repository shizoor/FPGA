...
class vgasignal(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        image = Image.open('pictures/rainbowsmiley.png')   #open the graphic.     
        maxsize = (267, 480)
        ...
        with m.If((self.framecounter[8])==1):
            m.d.comb+=xstretchrate.eq(int(256*imgarr.shape[1]/maxsize[0])-self.framecounter)    #Makes it animate.   
            m.d.comb+=ystretchrate.eq(int(256*imgarr.shape[0]/maxsize[1])-self.framecounter)    #remove the + and - framecounter and the if statements
        with m.Else():                                                                          #for a static image
            m.d.comb+=xstretchrate.eq(int(256*imgarr.shape[1]/maxsize[0])+self.framecounter)
            m.d.comb+=ystretchrate.eq(int(256*imgarr.shape[0]/maxsize[1])+self.framecounter)
        ...
        with m.If(self.v_line == 0):
            with m.If(self.h_timer == 0):
                self.framecounter.eq(self.framecounter+1)
